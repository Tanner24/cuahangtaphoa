...
function StoresPage() {
    ...
    const handleResetPassword = async (e) => {
        e.preventDefault();
        try {
            await api.resetStorePassword(showResetPassword.id, newPassword);
            setShowResetPassword(null);
            setNewPassword('');
            toast.success(`Đã đặt lại mật khẩu cho "${showResetPassword.name}"`);
        } catch (err) {
            toast.error(err.message);
        }
    };

    const handlePosAccess = async (store) => {
        try {
            // Xác định URL cơ sở của POS
            let posBaseUrl = import.meta.env.VITE_POS_CLIENT_URL || 'https://taphoaepos.vercel.app';

            // Nếu đang chạy ở localhost, ưu tiên dùng localhost cho POS nếu không có env
            if (window.location.hostname === 'localhost' && !import.meta.env.VITE_POS_CLIENT_URL) {
                posBaseUrl = 'http://localhost:5173'; // Cổng mặc định của Vite cho client thứ 2
            }

            const result = await api.getStoreLoginToken(store.id);
            if (result.success && result.token) {
                const posUrl = `${posBaseUrl}/login?token=${result.token}`;
                window.open(posUrl, '_blank');
            } else {
                window.open(posBaseUrl, '_blank');
            }
        } catch (err) {
            console.error(err);
            const posBaseUrl = import.meta.env.VITE_POS_CLIENT_URL || 'https://taphoaepos.vercel.app';
            window.open(posBaseUrl, '_blank');
            toast.error('Không thể tạo token tự động đăng nhập');
        }
    };
    ...
    return (
        <div>
                ...
                <div className="modal-overlay" onClick={() => setShowCreate(false)}>
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleCreate}>
                            ...
                            <div className="form-group">
                                <label className="form-label">Số điện thoại (Sẽ là tên đăng nhập)</label>
                            ...
                            <div className="modal-footer">
                                ...
                                <button type="submit" className="btn btn-primary">Tạo cửa hàng</button>
                            </div>
                        </form>
                    </div>
                </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    {canSales && (
                                        <>
                                            ...
                                            <button className="btn btn-warning" style={{ width: '100%', justifyContent: 'center', backgroundColor: '#f59e0b', color: '#fff', border: 'none' }} onClick={() => {
                                                setShowResetPassword(showEdit);
                                                setShowEdit(null);
                                            }}>
                                                <span className="material-icons" style={{ fontSize: 18, marginRight: 8 }}>lock_reset</span>
                                                Đặt lại mật khẩu
                                            </button>
                                        </>
                                    )}
            ...
            {canSales && showResetPassword && (
                <div className="modal-overlay" onClick={() => setShowResetPassword(null)}>
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2>🔐 Đặt lại mật khẩu: {showResetPassword.name}</h2>
                            <button className="btn btn-icon" onClick={() => setShowResetPassword(null)}>✕</button>
                        </div>
                        <form onSubmit={handleResetPassword}>
                            ...
                            <div className="modal-footer">
                                <button type="button" className="btn btn-secondary" onClick={() => setShowResetPassword(null)}>Hủy</button>
                                <button type="submit" className="btn btn-warning" style={{ backgroundColor: '#f59e0b', color: '#fff', border: 'none' }}>Xác nhận đặt lại</button>
                            </div>
                        </form>
                    </div>
                </div>
